...
def preorder():
    ...
    order = PreOrder(
        name=name, email=email, phone=phone,
        items=items, total=total, pickup_time=pickup_time
    )
    db.session.add(order)
    db.session.commit()
    
    return jsonify({'status': 'success', 'message': 'Pre-order placed successfully!'})
...
def get_reservations():
    ...
    return jsonify([{
        'id': r.id, 'name': r.name, 'date': r.date, 'time': r.time, 'guests': r.guests
